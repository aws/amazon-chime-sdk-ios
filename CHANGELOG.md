...
### Changed
* **Breaking** Every Objective-C-visible type, protocol and `NS_ENUM` generated from the SDK's Swift now carries an `AWSChime` prefix. Previously they were exposed under their bare Swift names, so building the SDK in the same target as another framework that declares a type with the same name failed with `'Logger' has different definitions in different modules`. **Swift is unaffected and requires no change** — Swift names are identical. **Objective-C** callers must add the prefix, for example `ConsoleLogger` becomes `AWSChimeConsoleLogger`, the `Logger` protocol becomes `AWSChimeLogger`, and enum constants such as `LogLevelINFO` become `AWSChimeLogLevelINFO`. `CwtEnum.h` is the one documented exception and keeps its current names, because its declarations are identical to the copy that ships in the prebuilt `AmazonChimeSDKMachineLearning` framework. See [Objective-C symbol migration](guides/objc_migration.md) for the complete mapping and that exception.
* **Breaking** `NSDictionary.toJsonString()` is exposed to Objective-C as `awsChimeToJsonString`. The Swift name is unchanged. A category method collides by selector rather than by type name, so a prefix on the type could not fix it.
* **Breaking** The `NSError` domain constants are Objective-C globals, so their names take the prefix too: `MediaErrorDomain` becomes `AWSChimeMediaErrorDomain`, and likewise for the other nine. The domain **string values** are unchanged, so comparing `error.domain` against a literal such as `@"AmazonChimeSDK.PermissionError"` keeps working.
* **Breaking** `AmazonChimeSDKMedia` 0.26.0 applies the same prefix to its own Objective-C surface, including `AudioClient`, `VideoClient`, `AppInfo` and the constants of its `NS_ENUM` types. Its Swift interface is likewise unchanged.
