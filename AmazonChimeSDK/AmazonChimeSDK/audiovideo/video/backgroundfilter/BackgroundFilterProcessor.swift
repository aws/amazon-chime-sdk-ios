...
public class BackgroundFilterProcessor {
    ...
    public init(logger: Logger) {
        self.logger = logger
        segmentationProcessor = SegmentationProcessor(logger: self.logger)
    }
    ...
    public func createForegroundAlphaMask(inputFrameCG: CGImage,
        ...
        if bufferPool == nil || inputFrameCG.width != bufferPoolWidth || inputFrameCG.height != bufferPoolHeight {
            ...
            do {
                ...
                try segmentationProcessor.initialize(height: segmentationProcessorHeight,
                                                     width: segmentationProcessorWidth,
                                                     channels: Int32(imageChannels))
            } catch ResourceError.notFound {
                logger.error(msg: "Unable to find selfie segmentation model." +
                             "See `Update Project File` section in README for more information on " +
                             "how to import `selfie_segmentation_landscape.tflite` as a bundle resource to your project.")
                return nil
            } catch {
        ...
        if segmentationProcessor.getModelState() != CwtModelState.LOADED {
        ...
        let predictResult: CwtPredictResult = segmentationProcessor.predict()
        if predictResult != CwtPredictResult.SUCCESS {
